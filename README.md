...
В целом, [можно заметить](#describe), что никаких выбросов нет и значения более или менее нормально распределены, поэтому оставляем их в изначальном виде.
...
+ *PR (Poverty Rate), UR (Unemployment Rate), PIPC (Personal Income per Capita), RGDP (Real GDP), Density, Immigrants, Marriage, Divorce, Temperature* - объясняющие переменные; 
